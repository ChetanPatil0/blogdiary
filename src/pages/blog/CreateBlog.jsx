...
//     .test(
//       "max-trimmed",
//       "Title must not exceed 50 characters (excluding spaces)",
//       (value) => !value || getTrimmedLength(value) <= 100
//     )
...
//       addNotification(`Maximum 5 additional images allowed (you selected ${files.length})`, "error");
//       return;
//     }
...
//       addNotification("Please fix validation errors before publishing", "error");
//       return;
//     }

//     if (!coverFile) {
//       setShowPublishModal(false);
//       addNotification("Please select a cover image", "error");
//       return;
//     }
...
//         trimmedValues.tags.forEach(tag => {
...
//           addNotification(response.data.message || "Blog created successfully!", "success");
//           resetForm();
//           setCoverSrc("");
//           setCoverFile(null);
//           setPostImages([]);
...
//                     placeholder={
//                       formik.values.tags.length >= 5 ? "Max tags reached" : "Type tag & press Enter or comma"
//                     }
...
//                       Drag & drop or click to select cover image
//                     </p>
...
//               <div>
//                 <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
//                   Additional Post Images (optional, max 5)
//                 </label>

//                 <div className="flex flex-wrap gap-4 mb-4">
//                   {postImages.map(({ preview }, idx) => (
//                     <div key={idx} className="relative w-24 h-24 rounded-lg overflow-hidden border border-slate-300 dark:border-slate-600">
//                       <img src={preview} alt={`Post image ${idx + 1}`} className="w-full h-full object-cover" />
//                       <button
//                         type="button"
...
//                   {postImages.length}/5 images selected
//                 </p>
//               </div>
...
//                   disabled={!hasAnyInput(formik.values)}
//                   className={`px-6 py-3 border rounded-lg font-medium transition ${
//                     hasAnyInput(formik.values)
//                       ? "text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 border-slate-300 dark:border-slate-600"
//                       : "text-slate-400 dark:text-slate-500 cursor-not-allowed opacity-50 border-slate-300 dark:border-slate-600"
//                   }`}
//                 >
//                   Cancel
//                 </button>
...
//                   disabled={formik.isSubmitting || !coverFile}
...
title: Yup.string()
  ...
  .test(
    "max-trimmed",
    "Title must not exceed 50 characters (excluding spaces)",
    (value) => !value || getTrimmedLength(value) <= 50
  )
  .max(100, "Title must not exceed 100 characters total"),
...
description: Yup.string()
  ...
  .test(
    "min-trimmed",
    "Description must be at least 10 characters (excluding spaces)",
    (value) => !value || getTrimmedLength(value) >= 10
  )
  .test(
    "max-trimmed",
    "Description must not exceed 300 characters (excluding spaces)",
    (value) => !value || getTrimmedLength(value) <= 300
  ),
...
content: Yup.string()
  ...
  .test(
    "min-trimmed",
    "Content must be at least 100 characters (excluding spaces)",
    (value) => !value || getTrimmedLength(value) >= 100
  )
  .test(
    "max-trimmed",
    "Content must not exceed 5000 characters (excluding spaces)",
    (value) => !value || getTrimmedLength(value) <= 5000
  ),
  category: Yup.string().required("Category is required"),
  tags: Yup.array()
    .of(Yup.string())
...
const CreateBlog = () => {
  ...
  const addTagsFromInput = () => {
    ...
    if (currentTags.length + uniqueNewTags.length > 5) {
      addNotification(`Cannot add ${uniqueNewTags.length} more tag(s). Maximum 5 allowed.`, "error");
      setTagInput("");
      return;
    }
  ...
  const handleSaveDraft = async () => {
    ...
    if (Object.keys(errors).length > 0) {
      ...
      addNotification("Please fix validation errors before saving draft", "error");
      return;
    }

    if (!coverFile) {
      addNotification("Cover image is required even for drafts", "error");
      return;
    }
            ...
            <Form className="space-y-7">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={
                      formik.values.tags.length >= 5
                        ? "Max tags reached"
                        : "Type tags separated by comma (e.g. tech, ai, react) then press Enter"
                    }
                    className="w-full px-4 py-3 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:border-primary-500 focus:ring-2 focus:ring-primary-200 dark:focus:ring-primary-800 outline-none transition disabled:opacity-60"
                  />
                  {formik.values.tags.length > 0 && (
              ...
              <div className="pt-6 border-t border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row gap-4 justify-end">
                ...
                <button
                  ...
                  disabled={formik.isSubmitting || !coverFile}
                  className="px-6 py-3 bg-slate-600 hover:bg-slate-700 text-white font-medium rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 min-w-[160px]"
                >
